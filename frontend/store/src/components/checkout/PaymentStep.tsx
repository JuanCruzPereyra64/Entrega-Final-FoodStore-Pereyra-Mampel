...
export function PaymentStep({ pedidoId, total, onSuccess, onBack }: PaymentStepProps) {
  ...
  const [mpData, setMpData] = useState<{ publicKey: string; email: string } | null>(null)
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setMpData({ publicKey: keyRes.public_key, email: user.email })
      } catch {
  ...
  useEffect(() => {
    if (!mpData || brickRef.current) return
    ...
    const mp = new (window as any).MercadoPago(mpData.publicKey, { locale: 'es-AR' })

    mp.bricks().create('cardPayment', 'cardPaymentBrick_container', {
      initialization: { amount: total, payer: { email: mpData.email } },
      callbacks: {
        onReady: () => {},
