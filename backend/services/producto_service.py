...
def create(uow: UnitOfWork, data: ProductoCreate) -> Producto:
    ...
    for ing in data.ingredientes:
        ...
        uow.productos.add_ingrediente_link(
            producto.id, ing.id, ing.cantidad, ing_model.unidad_medida_id
        )
...
def update(uow: UnitOfWork, producto_id: int, data: ProductoUpdate) -> Producto:
    ...
    if data.categoria_ids is not None:
        links_cat = uow.productos.get_categoria_links(producto_id)
    ...
    if data.ingredientes is not None:
        links_ing = uow.productos.get_ingrediente_links(producto_id)
        ...
        for ing in data.ingredientes:
            ing_model = ingrediente_service.get_by_id(uow, ing.id)
            uow.productos.add_ingrediente_link(
                producto_id, ing.id, ing.cantidad, ing_model.unidad_medida_id
            )
...
def add_ingrediente(uow: UnitOfWork, producto_id: int, ingrediente_id: int, cantidad: float = 1.0, unidad_medida_id: int = None) -> Producto:
    get_by_id(uow, producto_id)
    ingrediente_service.get_by_id(uow, ingrediente_id)
    
    existing = uow.productos.get_ingrediente_link(producto_id, ingrediente_id)
    if existing:
        raise HTTPException(status_code=400, detail="El ingrediente ya está asociado al producto")
    
    if unidad_medida_id is None:
        ing_model = ingrediente_service.get_by_id(uow, ingrediente_id)
        unidad_medida_id = ing_model.unidad_medida_id
    
    uow.productos.add_ingrediente_link(producto_id, ingrediente_id, cantidad, unidad_medida_id)
    uow.flush()
    return get_by_id(uow, producto_id)
