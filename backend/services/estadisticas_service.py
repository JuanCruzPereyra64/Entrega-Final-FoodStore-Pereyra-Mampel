...
def get_ventas_periodo(session: Session, desde: date, hasta: date, agrupacion: str = "day") -> list[VentasPeriodoItem]:
...
def get_productos_top(session: Session, limit: int = 10) -> list[ProductoTopItem]:
    stmt = (
        select(
...
def get_resumen_kpis(session: Session) -> ResumenResponse:
